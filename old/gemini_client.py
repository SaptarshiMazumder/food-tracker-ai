...
def prepare_image_part(client: genai.Client, path: str):
    """
    Developer API (api-key): prefer File API (client.files.upload),
    Vertex (ADC): fall back to inline bytes.
    """
    # Try File API first (works in API key mode; also works on Vertex if enabled)
    try:
