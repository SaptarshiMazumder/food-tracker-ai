...
def node_ing_quant(state: S) -> S:
    ...
    if "error" in res:
        ...
        print(f"[ing_quant] ❌ error in {state['timings']['ing_quant_ms']} ms")
        return state

    state["items"] = res.get("items", [])
    state["total_grams"] = float(res.get("total_grams") or 0.0)
    state["ing_conf"] = float(res.get("confidence", 0.0))
    state["ing_notes"] = res.get("notes")

    # Print compact summary
    n_items = len(state["items"])
    oil = next((x for x in state["items"] if "oil" in (x.get("name",""))), None)
    oil_g = f"{oil.get('grams',0)} g" if oil else "n/a"
    print(f"[ing_quant] ✅ items={n_items}  total_grams={state['total_grams']:.0f} g  "
          f"conf={state['ing_conf']:.2f}  oil={oil_g}  "
          f"took {state['timings']['ing_quant_ms']} ms")
    return state
